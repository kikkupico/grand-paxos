...
SHEETS = {"IV-benches": "legislators: faces, costume, shoulder bags, ledgers", "IV-legislators": "legislators and priests: faces, costume, hourglass, stone seats",
          "IV-ledger": "the ledger: a parchment strip on two rods, closed and open", "IV-messengers": "messengers: costume, satchels, message scrolls",
...
CANON = ("Canon: every ledger is a single strip of parchment wound between two wooden rods, never a bound book or a single sheet: carried closed, "
         "it is two rolls side by side tied with a cord, their four rod-ends knobbed. The Great Round is warm limestone ashlar and marble, not whitewash. "
...
def prompt_for(pid, anchor):
    ...
    lines = [f"- the {f['color']} mannequin ({where(f['screen'])}): {f['label']}" for f in shot["figures"] if f.get("in_frame")]
    ...
    text = (f"Image 1 is a 3D layout render for one comic panel. Redraw it as a finished comic panel in the style of a {STYLE}.\n"
            "Keep image 1's camera, framing, horizon and perspective exactly. Keep every piece of architecture, terrain, sea and sky where it is "
