...
def build(ground, M0, report):
    ...
    stair_half_at = lambda rr: (STAIR_W / 2) / rr
    passage_half_at = lambda rr: (GATE_W / 2 + .35) / rr
    for i, (r_in, r_out, top, rise) in enumerate(rows):
        upper = i >= LOWER_ROWS
        cuts = sorted([(a, stair_half_at((r_in + r_out) / 2)) for a in stair_angles] +
                      ([(g, passage_half_at((r_in + r_out) / 2)) for g in gates] if upper else []))
        for j, (a, h) in enumerate(cuts):
            a2, h2 = cuts[(j + 1) % len(cuts)]
            a0, a1 = a + h, a2 - h2 + (TAU if j == len(cuts) - 1 else 0)
            k.ring(X, Y, r_in, ver_r, z_orch - 1, top, M["tier"], 10, a0, a1)
            k.ring(X, Y, r_in, r_in + .09, top - .06, top + .02, M["marble"], 10, a0, a1)
    # stairs: individual steps, orchestra to verandah
    for a in stair_angles:
        lo = z_orch
        for (r_in, r_out, top, rise) in rows + [(ver_r, ver_r + .9, z_ver, z_ver - rows[-1][2])]:
            n = max(1, math.ceil(rise / .3 - 1e-6))
            for j in range(n):
                zz = lo + (top - lo) * (j + 1) / n
                depth = min(r_out - r_in, TREAD) / n
                k.box(*P(r_in + depth * (j + .5), a), z_orch - 1, zz, depth, STAIR_W, a, M["marble"])
            lo = top
    ...
    k.ring(X, Y, ver_r, WALL_IN, z_orch - 1, z_ver, M["pave"], 128)
    k.ring(X, Y, ver_r, ver_r + .12, z_ver - .08, z_ver + .02, M["marble"], 128)
    col_r = ver_r + .45; n_cols = int(TAU * col_r / 2.6)
    ...
    # gateways
    doors_open = {0: True}                                                                          # the east gate stands open toward the banquet house
    ...
    for n, (r_, a_, face) in enumerate(((31, .12, math.pi), (31.8, -.1, math.pi), (33, .02, math.pi + .3))):
        fx, fy = P(r_, a_); figure(people, fx, fy, ground.z(fx, fy), a_ + face, "stand", M["chiton"], cloth[n], M["skin"])
    ...
    info = {"rim": rim, "z_orch": z_orch, "z_verandah": z_ver, "seat_rows": len(rows), "tiers_incl_verandah": len(rows) + 1,
            ...
            "stair_riser_max_m": .3, "door_height_m": DOOR_H, "drop_bar_height_m": BAR_Z, "statues": statues, "figure_m": FIGURE_H,
...
def checks(info, round_ob, terrain, banquet_xyz, port_xyz, extra=()):
    ...
    res.update({"banquet_window_bearing_deg": round(win_b, 1), "quays_window_bearing_deg": round(win_p, 1),
                ...
                "seat_risers_comfortable": all(.35 <= r_ <= .65 for r_ in info["seat_risers_m"]), "stair_riser_climbable": info["stair_riser_max_m"] <= .3,
