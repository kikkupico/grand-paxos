...
## Commands
- `python3 tools/island_maps.py [a b c d e]` regenerates. Pass only the option you changed: A–D are frozen outputs from older generator versions. It regenerates the maps, heightmaps, site JSON and HTML blocks (about 10 s). It prints the site elevations and the sightline checks.
...
- World coordinates are metres, with x pointing east and y pointing south: 8000 × 5500 for A–D, 11000 × 8000 for E (`set_world`). One SVG unit is 5 m. The heightmap maps −120 m to 0 and +480 m to 65535.
