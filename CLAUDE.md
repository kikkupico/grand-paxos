...
- `tools/blender_buildings.py` is the full scene build: terrain (via `blender_terrain.build()`), the key buildings, and every other site through `tools/blender_sites.py`. Shared helpers live in `tools/blender_kit.py`. Run `~/.local/bin/blender -b -P tools/blender_buildings.py [-- --render]`. `blender/buildings-checks.json` and `blender/sites-checks.json` are committed.
