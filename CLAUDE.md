...
- `tools/blender_terrain.py` builds the Blender scene, `blender/paxos.blend` (gitignored), from the heightmap and site list. Run `~/.local/bin/blender -b -P tools/blender_terrain.py [-- --render]`; renders go to `renders/` (gitignored). `blender/terrain-checks.json` is committed.
