...
- `tools/blender_buildings.py` is the full scene build: terrain (via `blender_terrain.build()`) plus the key buildings. Run `~/.local/bin/blender -b -P tools/blender_buildings.py [-- --render]`. `blender/buildings-checks.json` is committed.
